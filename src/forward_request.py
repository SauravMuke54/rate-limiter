...
async def forward_request(upstream: str, request: Request) -> Response:
    ...
    try:
        ...
        logger.exception("Upstream timeout: %s %s", request.method, url)
        return Response(content=b"Upstream request timed out", status_code=504)
    except httpx.ConnectError:
        logger.exception("Upstream connection failed: %s %s", request.method, url)
    ...
    response_headers = {
        k: v for k, v in response.headers.items() if k.lower() not in EXCLUDED_RESPONSE_HEADERS
    }
